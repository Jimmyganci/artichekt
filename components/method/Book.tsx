...
function Book() {
  ...
  const [marginLeft, setMarginLeft] = useState('-400px')

  const onFlip = useCallback((page: number) => {
  ...
  return (
    <div
      className="flex flex-col items-center justify-center min-h-screen p-4 mt-16 relative transition-all duration-500"
      style={{marginLeft: marginLeft}} // Transition fluide ici
    >
      <div className="absolute flex flex-col space-y-12 z-10">
        {[...Array(5)].map((_, index) => (
          <div key={index} className="w-20 h-8 bg-primary rounded-full"></div>
        ))}
      </div>
      <HTMLFlipBook
        ref={bookRef}
      ...
      >
        <div
          onClick={() => onFlip(0)}
          className="demoPage bg-white p-4 text-center border-[25px] border-primary rounded-3xl"
        >
        ...
        <div className="demoPage bg-white px-6 border-[25px] border-primary rounded-3xl"></div>
        ...
        <div
          onClick={() => onFlip(46)}
          className="demoPage bg-white px-6 border-[25px] border-primary rounded-3xl"
        >
      ...
      {marginLeft === '0px' && (
        <button onClick={(e) => goToPage(3, e)} className="mt-6 text-primary">
          SOMMAIRE
        </button>
      )}
    </div>
