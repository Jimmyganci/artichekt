  ...
  return (
    <nav aria-label="breadcrumb" className="text-sm text-gray-600 mb-6">
      <ol className="flex flex-wrap p-0">
        <li>
          <Link href="/" className="hover:underline font-fontBold">Menu</Link>
        </li>
        {crumbs.map((crumb, i) => (
          <li key={crumb.href} className="flex items-center p-0">
            <span className="mx-3">/</span>
            {i === crumbs.length - 1 ? (
              <span className="font-semibold text-gray-900">{crumb.label.charAt(0).toUpperCase() + crumb.label.slice(1)}</span>
            ) : (
